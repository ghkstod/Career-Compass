...
def load_education_info(start_date,ncs_code, online_only=False, without_fee=False):
    ...
    filtered_df = df[df[' NCS 직무분류코드'] == ncs_code]

    if online_only:
        filtered_df=filtered_df[filtered_df['온라인여부'] == '온라인']
...
def main():
    ...
    st.markdown('<h2 style = "color : #494D46; font-size : 15px; text-align : right;">사용자의 성향을 기반으로 한<br> 직업, 국비 교육 프로그램 및 채용 공고 추천 서비스 구현</h2>', unsafe_allow_html=True)

    # 메뉴 사이드바
    st.sidebar.markdown('<h1 style = "color: black; font-size : 20px;">MENU🧭</h1>', unsafe_allow_html=True)
    selected_option = st.sidebar.selectbox('',['Home','나와 맞는 직무 찾기', '교육 과정 추천 받기'])
    

    # 교육 과정 추천 페이지
    if selected_option == '교육 과정 추천 받기':
                                ...
                                with cols[col_idx]:
                                    ...
                                    st.markdown(f'''
                                        <div style ="background-color :#cbf3f0; padding : 10px; border-radius : 5px;">
                                            ...
                                            <p style = "color : #494D46; "font-size : 15px;">수강 방식: {info['온라인여부']}</p>
